...
export default function Home() {
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f9f9f9', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
      ...
      <button
        style={{ fontSize: '1.25rem', padding: '1rem 2rem', borderRadius: '1rem', backgroundColor: 'white', color: '#0070f3', fontWeight: 'bold', cursor: 'pointer' }}
...
export function ChaptersPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#0070f3', padding: '1.5rem', color: 'white' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', maxWidth: '600px', margin: '0 auto' }}>
        {chapters.map((chapter) => (
          <div
            key={chapter.id}
            style={{ backgroundColor: 'white', color: '#0070f3', padding: '1rem', textAlign: 'center', borderRadius: '0.5rem', cursor: 'pointer', fontWeight: 'bold' }}
      ...
      <div style={{ textAlign: 'center', marginTop: '2rem' }}>
        <button onClick={() => router.push("/")} style={{ padding: '0.75rem 1.5rem', fontWeight: 'bold', backgroundColor: 'white', color: '#0070f3', borderRadius: '0.5rem', border: 'none' }}>العودة للرئيسية</button>
      </div>
    </div>
  );
}
...
export function ChapterPage({ chapterId }) {
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#0070f3', padding: '1.5rem', textAlign: 'center', color: 'white' }}>
      <h2 style={{ fontSize: '2rem', fontWeight: 'bold', marginBottom: '2rem' }}>اختر نوع الأسئلة</h2>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', alignItems: 'center' }}>
        <button onClick={() => handleClick("theory")} style={{ padding: '0.75rem 1.5rem', fontSize: '1rem', fontWeight: 'bold', borderRadius: '0.5rem', backgroundColor: 'white', color: '#0070f3', border: 'none' }}>أسئلة نظرية</button>
        <button onClick={() => handleClick("practical")} style={{ padding: '0.75rem 1.5rem', fontSize: '1rem', fontWeight: 'bold', borderRadius: '0.5rem', backgroundColor: 'white', color: '#0070f3', border: 'none' }}>أسئلة عملية</button>
        <button onClick={() => router.push("/chapters")} style={{ marginTop: '2rem', padding: '0.5rem 1rem', fontWeight: 'bold', borderRadius: '0.5rem', backgroundColor: 'white', color: '#0070f3', border: 'none' }}>⬅ العودة للأبواب</button>
      </div>
    </div>
  );
}
...
function QuestionRenderer({ questions }) {
  ...
  if (current >= questions.length) {
    return (
      <div style={{ padding: '1.5rem', textAlign: 'center', backgroundColor: '#0070f3', color: 'white', minHeight: '100vh' }}>
        <h2 style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>تم الانتهاء من جميع الأسئلة 🎉</h2>
        <button onClick={() => router.push("/")} style={{ marginTop: '1.5rem', padding: '0.75rem 1.5rem', fontWeight: 'bold', backgroundColor: 'white', color: '#0070f3', border: 'none', borderRadius: '0.5rem' }}>العودة للرئيسية</button>
      </div>
    );
  }
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', alignItems: 'center' }}>
        {q.options.map((option, index) => (
          <button
            ...
            disabled={showResult}
            style={{ padding: '0.75rem 1.5rem', borderRadius: '0.5rem', backgroundColor: 'white', color: '#0070f3', cursor: 'pointer', minWidth: '200px', fontFamily: 'monospace', fontWeight: 'bold' }}
          >
            {option}
          </button>
        ))}
      ...
      {showResult && (
        <div style={{ marginTop: '1.5rem', fontWeight: 'bold' }}>
          ...
          <div style={{ marginTop: '1rem', display: 'flex', gap: '1rem', justifyContent: 'center' }}>
            <button onClick={prevQuestion} style={{ padding: '0.5rem 1rem', backgroundColor: 'white', color: '#0070f3', border: 'none', borderRadius: '0.5rem', fontWeight: 'bold' }}>⬅ السابق</button>
            <button onClick={nextQuestion} style={{ padding: '0.5rem 1rem', backgroundColor: 'white', color: '#0070f3', border: 'none', borderRadius: '0.5rem', fontWeight: 'bold' }}>التالي ➡</button>
          </div>
        </div>
      )}
    </div>
